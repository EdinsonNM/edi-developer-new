...
import { ArrowLeft, ExternalLink, Monitor, Sparkles } from "lucide-react";
...
function AppBadge({
  ...
  const styles: Record<string, string> = {
    ...
    comingSoon: "bg-amber-100 text-amber-800 border-amber-200",
      ...
      <div className="space-y-8">
        ...
        <Card className="overflow-hidden border-slate-200 shadow-lg">
          <div className="grid md:grid-cols-[1fr_1.2fr] gap-0">
            <div className="relative aspect-video md:aspect-auto md:min-h-[240px] bg-linear-to-br from-slate-100 to-slate-200 flex items-center justify-center p-6">
              {!slaimImageError ? (
                <img
                  src="/apps/slaim.png"
                  alt={t.slaimTitle}
            ...
            <CardContent className="flex flex-col justify-center p-8">
              ...
              <p className="text-slate-600 mb-4">{t.slaimDescription}</p>
              <ul className="list-none space-y-2 mb-6 text-slate-600">
        ...
        <Card
          id="emotional-ai"
          className="overflow-hidden border-slate-200 shadow-lg scroll-mt-24"
        >
          <div className="grid md:grid-cols-[1fr_1.2fr] gap-0">
            <CardContent className="flex flex-col justify-center p-8 order-2 md:order-1">
              ...
              <p className="text-slate-600 mb-6">{t.emotionalAiDescription}</p>
              <Button asChild size="lg" className="rounded-full w-fit">
                <a
            ...
            <div className="relative aspect-video md:aspect-auto md:min-h-[240px] bg-white flex items-center justify-center p-6 order-1 md:order-2 border-b md:border-b-0 md:border-l border-slate-200">
          ...
          <Card className="border-slate-200 border-dashed bg-slate-50/50">
            <CardContent className="p-6 flex flex-col items-center justify-center min-h-[180px] text-center">
              ...
              <h3 className="font-semibold text-slate-900 mb-2">
                {t.placeholderAppTitle}
              </h3>
              <p className="text-sm text-slate-600">{t.placeholderAppDesc}</p>
            </CardContent>
          </Card>
          <Card className="border-slate-200 border-dashed bg-slate-50/50">
            <CardContent className="p-6 flex flex-col items-center justify-center min-h-[180px] text-center">
              <Sparkles className="h-10 w-10 text-slate-400 mb-4" aria-hidden />
              <h3 className="font-semibold text-slate-900 mb-2">
                {t.moreToolsComing}
              </h3>
              <p className="text-sm text-slate-600">{t.placeholderAppDesc}</p>
