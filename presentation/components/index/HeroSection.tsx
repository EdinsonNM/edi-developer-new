...
import { HERO_ICONS } from "./hero-icons";
  ...
  useEffect(() => {
    ...
    const loadHyperspeed = () => {
      if ("requestIdleCallback" in window) {
        requestIdleCallback(() => setShouldLoadHyperspeed(true));
      } else {
        setTimeout(() => setShouldLoadHyperspeed(true), 1000);
      }
    };
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 w-full flex flex-col items-center">
        {/* Main Headline - slide-up sin opacity 0 para no retrasar LCP */}
        <h1 className="max-w-5xl text-5xl font-bold tracking-tight text-slate-900 sm:text-7xl md:text-8xl mb-6 animate-slide-up text-center">
          {t.heroTitle} <br />
          <span className="text-slate-500">{t.heroSubtitle}</span>
        </h1>

        {/* Subtitle */}
        <p className="max-w-2xl text-lg text-slate-600 mb-10 animate-fade-in-up opacity-0 [animation-delay:150ms] [animation-fill-mode:forwards] text-center">
          {t.heroDescription}
        </p>

        {/* CTA Buttons */}
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-20 animate-fade-in-up opacity-0 [animation-delay:300ms] [animation-fill-mode:forwards]">
          <a
            href="#edi-academy"
            onClick={(e) => {
              e.preventDefault();
        ...
        <div
          className="w-full max-w-7xl mx-auto animate-fade-in-up opacity-0 [animation-delay:450ms] [animation-fill-mode:forwards]"
          aria-label={
            language === "es"
